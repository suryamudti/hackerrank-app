...
class ObserveProgressOverviewUseCaseTest {
    ...
    private val useCase = ObserveProgressOverviewUseCase(progressRepository, contentRepository)
    ...
    @Test
    fun `invoke computes category mastery and mastered count`() = runTest {
    ...
    @Test
    fun `invoke handles empty progress`() = runTest {
        every { contentRepository.getAllStructures() } returns flowOf(structures)
