...
class FinishQuizUseCaseTest {
    ...
    @Test
    fun `invoke persists new progress and records gamification`() =
        runTest {
            val session =
                QuizSession(
                    questions = questions,
                    answers = mapOf("q1" to 0, "q2" to 0),
            ...
            coEvery { progressRepository.insertQuizResult(any(), any(), any(), any()) } returns Unit
    ...
    @Test
    fun `invoke merges with existing progress`() =
        runTest {
            val session =
                QuizSession(
                    questions = questions,
            ...
            val gamificationResult =
                GamificationResult(
                    ...
                    previousLevel = 1,
                    newBadges = emptyList(),
                    streakInfo = StreakInfo(2, 2, true, null, "2026-07-05"),
                )
            coEvery {
