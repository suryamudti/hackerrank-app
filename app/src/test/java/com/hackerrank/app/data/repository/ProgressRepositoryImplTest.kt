...
class ProgressRepositoryImplTest {
    ...
    private val quizResultDao: QuizResultDao = mockk()
    private val gson: Gson = Gson()
    private val context: Context = mockk(relaxed = true)
    private val repository = ProgressRepositoryImpl(progressDao, profileDao, quizResultDao, gson, context)

    private val sampleProgressEntity =
        UserProgressEntity(
            structureId = "struct1",
            quizzesCompleted = 5,
            totalCorrect = 15,
    ...
    private val sampleProfileEntity =
        UserProfileEntity(
            id = 1,
            totalXp = 1200,
            currentStreak = 3,
    ...
    @Test
    fun `getProgressByStructureId maps entity to domain correctly`() =
        runTest {
    ...
    @Test
    fun `getProgressByStructureId returns null when not found`() =
        runTest {
    ...
    @Test
    fun `getAllProgress maps entities to domain list correctly`() =
        runTest {
    ...
    @Test
    fun `upsertProgress calls progressDao upsert`() =
        runTest {
    ...
    @Test
    fun `getProfile maps entity to domain correctly`() =
        runTest {
    ...
    @Test
    fun `getProfileSync maps entity correctly`() =
        runTest {
    ...
    @Test
    fun `upsertProfile serializes badges and calls profileDao upsert`() =
        runTest {
    ...
    @Test
    fun `getMasteredCount returns flow of count`() =
        runTest {
