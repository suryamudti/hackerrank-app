...
class RecordDailyChallengeUseCase
    @Inject
    constructor(
        ...
        suspend operator fun invoke(
            ...
            if (dailyChallengeRepository.isDailyChallengeCompleted(today)) return null

            problemRepository.markAsSolved(problemId)
            val result = gamificationEngine.recordDailyChallengeCompleted(difficulty, bonusXp)
            dailyChallengeRepository.setDailyChallengeCompleted(today)
